...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'photos' | 'settings'>('dashboard');
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="border-l border-slate-200 pl-3">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-wider leading-none">
                  {userRole === 'ADMIN' ? '365ボイス マスターコントロール' : '365ボイス 代理店コントロール'}
                </p>
      ...
      <div className="flex-1 flex flex-col sm:pl-60">
        ...
        {activeTab === 'settings' && (
          isLoading || !settings ? (
            <div className="bg-white border border-slate-200/80 rounded-2xl py-16 px-4 text-center space-y-3 shadow-sm">
              <RefreshCw className="w-8 h-8 text-indigo-500 animate-spin mx-auto" />
              <p className="text-xs font-extrabold text-slate-600">自動投稿・キーワード設定を読み込み中...</p>
            </div>
          ) : (
